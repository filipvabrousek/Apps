...
class ViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CBCentralManagerDelegate, CBPeripheralDelegate, UIGestureRecognizerDelegate, Reset, LOC, CadD {
    ...
    lazy var mainTitles: UIStackView = {
        let m = MainTitles()
        return m
    }()
    
    lazy var main: UIStackView = {
        let v = MainStack()
        return v
    }()
    
    
    var distSubline: UILabel = {
        let l = Flabel(text: "Distance", font: .boldSystemFont(ofSize: 14), color: .gray, align: .center)
        return l.gen()
    }()
...
// var BLBack = View(background: .white, hidde: true)
    
    
    var bllabel: UILabel = {
        let l = UILabel()
        l.font = UIFont.systemFont(ofSize: 34, weight: UIFont.Weight.heavy)
        l.text = "Accessories"
        l.isHidden = true
        return l
    }()
    ...
    lazy var bluetoothView: UITableView = {
        let tv = BluetoothTable()
        tv.delegate = self
        tv.dataSource = self
        return tv
    }()
    
    lazy var ppicker: Pickerb = {
        let b = Pickerb()
        b.addTarget(self, action: #selector(self.shot(sender:)), for: .touchUpInside)
        return b
    }()
    
    lazy var showbl: ShowBL = {
        let b = ShowBL()
        b.addTarget(self, action: #selector(self.pair(sender:)), for: .touchUpInside)
        return b
    }()
    ...
    lazy var startMain: AddB = {
        let b = AddB()
        b.titleLabel?.font = UIFont.boldSystemFont(ofSize: 17)
        b.setTitle("Start pacing", for: [])
        b.isHidden = true
        b.backgroundColor = .black
        b.addTarget(self, action: #selector(self.startSection(sender:)), for: .touchUpInside)
        return b
    }()
    ...
    lazy var segments: UISegmentedControl = {
        let seg = Seg(items: ["Map", "Music"], selector: #selector(self.switchSegment(_:)))
        return seg.genFirst()
    }()
    ...
    lazy var startBtn: StartButton = {
        var b = StartButton()
        b.setTitle("Start", for: [])
        b.titleLabel?.font = UIFont.boldSystemFont(ofSize: 18)
        b.addTarget(self, action: #selector(self.start(sender:)), for:.touchUpInside)
        return b
    }()
    
    
    lazy var songsBtn: ShareButton = {
        var b = ShareButton()
        b.setTitle("M", for: [])
        b.titleLabel?.font = UIFont.systemFont(ofSize: 18, weight: UIFont.Weight.heavy)
        b.addTarget(self, action: #selector(self.switcho(sender:)), for: .touchUpInside)
        return b
    }()
