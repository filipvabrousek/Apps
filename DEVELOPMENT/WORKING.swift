...
class ShareViewController: UIViewController, UITextViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    // let t = Timer(timeInterval: 0.5, target: self, selector: #selector, userInfo: <#T##Any?#>, repeats: <#T##Bool#>)
...
/*------------------------------------------------------LIFECYCLE------------------------------------------------------*/
        ...
        PHImageManager.default().requestImage(for: asset, targetSize: realsize , contentMode: .aspectFit, options: options) { (image, info) in
            ...
            if image != nil {
                ...
                if run.sectionDist != "" && run.sectionSecs != 0 {
                    ...
                    let pace = Pace(sec: Double(ss), dist: km)
                    i = self.g.genImage(image: image!, distance: run.distance, time: write, efpace: pace.getPace(), g1: self.grad, g2: self.light)
                    ...
                    // let pace = Pace(sec: Double(ss), dist: Double(dd)! / 1000.0)
                    i = self.g.genImage(image: image!, distance: run.distance, time: write, efpace: "", g1: self.grad, g2: self.light)
                    self.gpace = ""//pace.getPace()
                }
                ...
                self.imgview.image = i
                /*  chosen = genImage(image: chosen, text: run.distance, time: write)
                 imgview.image = chosen*/
            }
        }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let d = UserDefaults.standard.value(forKey: "isFirst") as? Bool{
            if d == true {
                switcher.setOn(true, animated: true) // only once
...
extension ShareViewController {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        /*
         if s != nil {
         let i = genImage(image: s!, text: gdist, time: gtime)
         imgview.image = i
         ...
         let i = self.genImage(image: image!, text: run.distance, time: write)
